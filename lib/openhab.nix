{
  ...
  compact = list:
    filter (e: (e != null) && (e != "")) list;

  listToText = list:
    concatStringsSep " " list;

  attrsToText = quote: attrs: let
    sep =
      if (length (attrNames attrs) > 1)
      then "\n"
      else " ";
  in
    concatStringsSep (
    ...
    (lib.mapAttrsToList
      ...
      in "${optionalString quote "  "}${k}=${
        if quote
        then toQuotedText val
        else toPlainText v
      }")
      attrs);
  ...
  # the functions under versions return as string
  isVX = version:
    (lib.versions.majorMinor cfg.package.version) == version;
  isV2 = isV2dot5;
  isV2dot5 = isVX "2.5";
  isV3 = isV3dot1 || isV3dot2 || isV3dot3;
  isV3dot1 = isVX "3.1";
  isV3dot2 = isVX "3.2";
  isV3dot3 = isVX "3.3";
  ...
  keyPrefix =
    if isV2
    then "org.eclipse.smarthome"
    else if isV3
    then "org.openhab"
    else "ERROR_unknown_version";

  itemName = listOrStr:
    if builtins.isList listOrStr
    then itemName (concatStringsSep "_" listOrStr)
    else toLower (replaceWithChar ["____" "___" "__"] "_" (replaceWithChar [":" "-" "," "(" ")" "'" " "] "_" listOrStr));
    ...
    (
      ...
      (
        ...
        ++ optional (has t.params) (
          let
            sep =
              if (length (attrNames t.params) > 1)
              then "\n"
              else " ";
          in ("[${sep}" + (attrsToQuotedText t.params) + "${sep}]")
        )
  ...
  bridgeName = listOrStr:
    ...
    else replaceStrings [" "] ["_"] listOrStr;
      ...
      (e:
        listToText (
          ...
          ++ optional (has e.params) (
            let
              sep =
                if (length (attrNames e.params) > 1)
                then "\n"
                else " ";
            in ("[" + sep + (attrsToQuotedText e.params) + sep + "]")
          )
        ))
      t.channels);
in rec {
  ...
  openhab = {
    ...
    in
      listToText (
        ...
        ++ optional (has finalSettings) (
          let
            sep =
              if (length (attrNames i.settings) > 1)
              then "\n"
              else " ";
          in
            "{${sep}"
    ...
    itemFileName = item:
      lib.toLower (lib.concatStringsSep "_" [item.type item.name]) + ".items";

    thingFileName = thing:
      lib.toLower (lib.concatStringsSep "_" (compact (with thing; [type binding subtype id]))) + ".things";
  ...
  mergeChildren = attrs: list:
    map (e: lib.recursiveUpdate attrs e) list;
}
